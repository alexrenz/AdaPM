...
class Customer {
 public:
   ...
   */
  using RecvHandle = std::function<bool(const Message& recved)>; // [sysChange] changed to bool return type (for message counts)
   ...
   */
  int NewRequest(int recver, int num=-1);
  ...
  /**
   * \brief return whether all responses are received for the request. threadsafe
   * \param timestamp the timestamp of the request
   */
  int HasAllResponses(int timestamp);

  /**
   * \brief add a number of responses to timestamp. threadsafe
   *        returns true if all responses for the timestamp were received
   */
  bool AddResponse(int timestamp, int num = 1);

  /**
   * \brief accept a received message from \ref Van. threadsafe
   * \param recved the received the message
   */
  inline void Accept(const Message& recved, bool placeFront=false) { // [sysChange] Allow to prioritze pin messages
    recv_queue_.Push(recved, placeFront);
  }

...
class KVWorker : public SimpleApp {
 public:
  ...
  /**
   * \brief empty constructor (for construction by ColoKV) [sysChange]
   */
  explicit KVWorker() : SimpleApp() {
  }
   ...
   */
  using Slicer = std::function<int( // [sysChange]: changed to return type int
  ...
  // count total and local kv requests [sysChange]
  unsigned long reqs_total = 0;
  unsigned long reqs_local = 0;

 protected:

  /**
   * \brief internal pull, C/D can be either SArray or std::vector
   * [sysChange]: now protected instead of private
   */
 ...
 private:
  ...
  bool Process(const Message& msg); // [sysChange]: return type bool (for Customer message counts)
  /** \brief default kv slicer */
  int DefaultSlicer(const KVPairs<Val>& send, // [sysChange]: return type int
                     const std::vector<Range>& ranges,
                     SlicedKVs* sliced);

  /** \brief retrieve the server responsible for a range [sysChange] */
  virtual inline int GetServerForRange(int range) {
    return Postoffice::Get()->ServerRankToID(range);
  }
...
template <typename Val>
bool KVServer<Val>::Process(const Message& msg) {
  if (msg.meta.simple_app) {
    SimpleApp::Process(msg); return true;
  }
  KVMeta meta;
...
void KVServer<Val>::Response(const KVMeta& req, const KVPairs<Val>& res) {
  ...
  if (res.keys.size()) {
    ...
    if (res.vals.size()) { // [sysChange]: there is cases where we send 1 key (as counter), but no vals
...
template <typename Val>
int KVWorker<Val>::DefaultSlicer(
    const KVPairs<Val>& send, const std::vector<Range>& ranges,
  ...
  return skipped; // returns the number of ranges that are skipped (because we don't request any keys from there) [sysChange]
}
...
void KVWorker<Val>::Send(int timestamp, bool push, int cmd, const KVPairs<Val>& kvs) {
  // slice the message
  SlicedKVs sliced;
  int skipped = slicer_(kvs, Postoffice::Get()->GetServerKeyRanges(), &sliced); // [sysChange]: skipped calculation was extra (below)

  // need to add response first, since it will not always trigger the callback
  if(skipped != 0) {
    obj_->AddResponse(timestamp, skipped);
  }
  if (obj_->HasAllResponses(timestamp)) { // in case we don't send out any messages, run the callback right away
...
template <typename Val>
bool KVWorker<Val>::Process(const Message& msg) {
  if (msg.meta.simple_app) {
    SimpleApp::Process(msg); return true;
  }
  ...
  return true; // return true to have the arriving message counted in customer
}
template <typename Val>
void KVWorker<Val>::RunCallback(int timestamp) {
  ...
  if (it != callbacks_.end()) {
    // found a callback. erase it and run it
...
int KVWorker<Val>::Pull_(
  ...
  AddCallback(ts, [this, ts, keys, vals, lens, cb]() mutable {
      ...
      if (vals->empty()) {
        ...
        LL << "Vals size is " << vals->size() << ", but totally we got " << total_val << " vals";

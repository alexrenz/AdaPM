...
void RunWorker(int customer_id, ServerT* server=nullptr) {
  ...
  if ((num_workers > 1 && worker_id ==1) || (num_workers == 1 && worker_id == 0)) {
    // read and send W
    sw["read_factors"].start();
    vector<Key> full_w_keys (num_rows);
    std::iota(full_w_keys.begin(), full_w_keys.end(), 0);
    std::vector<ValT> full_w = read_dense_matrix<std::vector<ValT>>(dataset + "W.mma", true);
  ...
  for(int epoch = 1; epoch != epochs+1; ++epoch) {
    ...
    for(uint subepoch = 0; subepoch != num_workers; ++subepoch) {
      ...
      for (unsigned long z=data.block_start(h_block); z!=data.block_end(h_block); ++z) {
        ...
        // get factors
        factors_keys[0] = dp.i;
        factors_keys[1] = dp.j+num_w_keys;
    ...
    if (compute_loss) {
      ...
      sw["loss"].stop();
      if (worker_id == 0) {
        ADLOG("(Ep." << epoch << ") Loss:\t" << loss_vec[0] << " (" << sw["loss"] << ")");
...
int process_program_options(const int argc, const char *const argv[]) {
  ...
  desc.add_options()
    ...
    ("lambda,l", po::value<double>(&lambda)->default_value(1), "Regularization parameter lambda")
    ...
    ("sc", po::value<bool>(&sc)->default_value(true), "use short circuit local updates")
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "dsgd. " << role << ": " << epochs << " epochs on " << dataset << "\n";
